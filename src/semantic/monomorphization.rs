...
impl Monomorphizer {
    pub fn new() -> Self {
        Self { monomorphized_funcs: HashMap::new(), counter: 0 }
    }
    ...
    fn mangle_type(ty: &Type) -> String {
        match ty {
            ...
            Type::Union(variants) => {
                let variants_mangled: Vec<String> =
                    variants.iter().map(|v| Self::mangle_type(v)).collect();
                format!("union_{}", variants_mangled.join("_"))
            }
            Type::GenericApp { name, type_args } => {
                let args_mangled: Vec<String> =
                    type_args.iter().map(|t| Self::mangle_type(t)).collect();
            ...
            Type::Fn(params, ret) => {
                let params_mangled: Vec<String> =
                    params.iter().map(|t| Self::mangle_type(t)).collect();
    ...
    fn create_substitution(type_params: &[String], type_args: &[Type]) -> HashMap<String, Type> {
        type_params
            .iter()
            .zip(type_args.iter())
    ...
    pub fn specialize_function(
        ...
        let (param_types, return_type) = match &original_symbol.kind {
            ...
            _ => {
                return crate::semantic::semantic_error(format!("{} is not a function", func_name))
            }
        };

        // Specialize parameter types
        let specialized_params: Vec<Type> =
            param_types.iter().map(|p| p.substitute(&subst)).collect();
    ...
    fn specialize_stmts(&self, stmts: &[Stmt], subst: &HashMap<String, Type>) -> Vec<Stmt> {
        stmts.iter().map(|stmt| self.specialize_stmt(stmt, subst)).collect()
    }
...
impl TypeChecker {
    ...
    pub fn contains_type_params(&self, ty: &Type) -> bool {
        match ty {
            ...
            Type::Fn(params, ret) => {
                params.iter().any(|p| self.contains_type_params(p))
                    || self.contains_type_params(ret)
            }
            Type::Result(ok, err) => {
            ...
            Type::GenericApp { type_args, .. } => {
                type_args.iter().any(|t| self.contains_type_params(t))
            }
