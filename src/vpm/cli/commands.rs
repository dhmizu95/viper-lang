...
pub fn add_dependency(
    ...
    let content =
        fs::read_to_string(manifest_path).map_err(|e| format!("Failed to read vpm.toml: {}", e))?;

    let mut manifest: toml::Value =
        toml::from_str(&content).map_err(|e| format!("Failed to parse vpm.toml: {}", e))?;
...
pub fn publish_package(bump: Option<&str>, dry_run: bool) -> Result<()> {
    ...
    let new_version = if let Some(level) = bump {
        let parts: Vec<u32> = current_version.split('.').filter_map(|s| s.parse().ok()).collect();
