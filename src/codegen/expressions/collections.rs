...
/// Generate tuple creation
pub fn generate_tuple<'ctx>(
    state: &mut CodeGenState<'_, 'ctx>,
    elements: &[Expr],
) -> Result<BasicValueEnum<'ctx>, String> {
    if elements.is_empty() {
        // Empty tuple returns 0 as a placeholder
    ...
    let elem_types: Vec<inkwell::types::BasicTypeEnum<'ctx>> = elem_values
        .iter()
        .map(|v| v.get_type())
        .collect();

    let struct_type = state.context.struct_type(&elem_types, false);
    
    // Create an alloca for the tuple struct
    let tuple_ptr = state
        .builder
        .build_alloca(struct_type, "tuple_storage")
        .map_err(|e| format!("Failed to allocate tuple: {:?}", e))?;

    // Store each element in the struct
    for (i, val) in elem_values.iter().enumerate() {
        let elem_ptr = state
            .builder
            .build_struct_gep(struct_type, tuple_ptr, i as u32, &format!("tuple_elem_{}", i))
