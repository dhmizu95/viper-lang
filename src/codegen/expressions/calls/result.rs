...
pub fn generate_ok_constructor<'ctx>(
    ...
    let result_struct_type = state
        .context
        .struct_type(&[state.context.i8_type().into(), state.context.i64_type().into()], false);

    // Allocate space for the Result struct
    let result_alloca =
        state.builder.build_alloca(result_struct_type, "ok_result").expect("alloca");
    ...
    let value_i64 = if value.is_int_value() {
        ...
        state
            .builder
            .build_ptr_to_int(value.into_pointer_value(), state.context.i64_type(), "ok_ptr_to_i64")
            .map_err(|e| format!("Failed to convert ptr to i64: {:?}", e))?
    } else {
        return crate::codegen::codegen_error(format!(
            "Unsupported Ok value type: {:?}",
            value.get_type()
        ));
    };
    ...
    let result_val =
        state.builder.build_load(result_struct_type, result_alloca, "ok_result_val").expect("load");
    Ok(result_val.into())
}
...
pub fn generate_err_constructor<'ctx>(
    ...
    // Allocate space for the Result struct
    let result_alloca =
        state.builder.build_alloca(result_struct_type, "err_result").expect("alloca");
    ...
    let result_val = state
        .builder
        .build_load(result_struct_type, result_alloca, "err_result_val")
        .expect("load");
    Ok(result_val.into())
}
