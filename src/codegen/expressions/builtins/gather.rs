...
pub fn generate_gather_call<'ctx>(
    ...
    let futures_array = state
        .builder
        .build_alloca(futures_array_type, "futures_array")
    ...
    let gather_func = state
        .module
        .get_function("vp_future_gather")
        .ok_or_else(|| "vp_future_gather not found".to_string())?;

    // Convert pointer to i64 for the gather function
    let futures_ptr_i64 =
        state.builder.build_ptr_to_int(futures_ptr, i64_type, "futures_ptr_i64").unwrap();

    let results_ptr = state
        .ir_builder
        .build_call(
            state.builder,
            gather_func,
    ...
    let result_list = state
        .ir_builder
        .build_call(state.builder, list_create, &[], "result_list")
        .unwrap()
        .into_pointer_value();

    // Unpack results and append to list
    let i64_ptr_type = state.context.ptr_type(inkwell::AddressSpace::default());
    let results_array_ptr =
        state.builder.build_int_to_ptr(results_ptr, i64_ptr_type, "results_array_ptr").unwrap();
    ...
    for i in 0..args.len() {
        ...
        let result_val =
            state.builder.build_load(i64_type, result_gep, &format!("result{}", i)).unwrap();
    ...
    let gather_free = state
        .module
        .get_function("vp_future_gather_free")
