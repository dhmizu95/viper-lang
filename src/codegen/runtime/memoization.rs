...
pub fn declare_memoization_functions<'ctx>(
    ...
    let lru_cache_get_type = i64_type.fn_type(&[i8_ptr_type.into(), i8_ptr_type.into(), context.i32_type().ptr_type(inkwell::AddressSpace::default()).into(), context.i32_type().ptr_type(inkwell::AddressSpace::default()).into()], false);
    let lru_cache_get = module.add_function("vp_lru_cache_get", lru_cache_get_type, None);

    let lru_cache_set_type = void_type.fn_type(
        &[i8_ptr_type.into(), i8_ptr_type.into(), i64_type.into(), i64_type.into(), context.i32_type().into()],
        false,
    );
    ...
    let cache_get_type = i64_type.fn_type(&[i8_ptr_type.into(), i8_ptr_type.into(), context.i32_type().ptr_type(inkwell::AddressSpace::default()).into(), context.i32_type().ptr_type(inkwell::AddressSpace::default()).into()], false);
