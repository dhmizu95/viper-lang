...
pub extern "C" fn vp_logging_logger_free(logger: *mut std::ffi::c_void) {
    if !logger.is_null() {
        unsafe { drop(std::ffi::CString::from_raw(logger as *mut i8)); }
    }
}
...
pub extern "C" fn vp_logging_enabled_for(logger: *mut std::ffi::c_void, level: i64) -> i64 {
    ...
    if level >= logger_level { 1 } else { 0 }
}
    ...
    unsafe {
        ...
        // Check if logger exists, create if not
        {
            ...
            if loggers.contains_key(&name_str) {
                return std::ffi::CString::new(name_str).unwrap().into_raw() as *mut std::ffi::c_void;
            }
        }
...
#[no_mangle]
pub extern "C" fn vp_logging_basic_config(level: i64, format: *const i8, stream: *mut std::ffi::c_void) {
    let _ = format;
    let _ = stream;
...
// Level constants
#[no_mangle]
pub extern "C" fn vp_logging_debug_level() -> i64 { 0 }

#[no_mangle]
pub extern "C" fn vp_logging_info_level() -> i64 { 1 }

#[no_mangle]
pub extern "C" fn vp_logging_warning_level() -> i64 { 2 }

#[no_mangle]
pub extern "C" fn vp_logging_error_level() -> i64 { 3 }

#[no_mangle]
pub extern "C" fn vp_logging_critical_level() -> i64 { 4 }

#[no_mangle]
pub extern "C" fn vp_logging_notset_level() -> i64 { 5 }
...
pub extern "C" fn vp_logging_filter_free(filter: *mut std::ffi::c_void) {
    if !filter.is_null() {
        unsafe { drop(std::ffi::CString::from_raw(filter as *mut i8)); }
    }
}
