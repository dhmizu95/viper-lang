...
/* ViperList - Unified 32-byte layout for predictable LLVM IR */
struct ViperList {
    int64_t length;         /* 0:  Current number of elements */
    int64_t capacity;       /* 8:  Allocated capacity (in elements) */
    ViperListType elem_type;/* 16: Element type for type-specific access */
    union {
    ...
    } data;                 /* 24: Union of data pointers */
};                          /* Total: 32 bytes */
...
/* ViperDict - Unified 32-byte layout */
struct ViperDict {
    int64_t size;         /* 0:  Bucket array size */
    int64_t count;        /* 8:  Number of entries */
    DictEntry** buckets;  /* 16: Bucket array pointer */
...
/**
 * ViperBytes - Immutable byte sequence
 */
typedef struct ViperBytes {
    uint8_t* data;          /* 0: Bytes data */
    int64_t len;            /* 8: Length */
} ViperBytes;               /* Total: 16 bytes */
...
typedef struct ViperString {
    union {
        ...
        struct {
            char* heap_data;        /* 0:  Pointer to heap data */
            int64_t length;         /* 8:  String length (positive, bit 63 is 0) */
        } heap;
        
        /* Small string (inline storage, SSO) */
        struct {
            char sso_data[15];      /* 0-14: Inline storage (15 bytes) */
            int8_t sso_len_flags;   /* 15:   Length (bits 0-6) and SSO flag (bit 7) */
        } sso;
    } data;
} ViperString;            /* Total: 16 bytes */

/* SSO flag: bit 63 of the second 64-bit word (offset 15) */
#define VIPER_SSO_FLAG_BIT 63
#define VIPER_SSO_FLAG ((int64_t)1 << VIPER_SSO_FLAG_BIT)

/* Check if string uses SSO (inline storage) */
static inline int vp_str_is_sso_inline(ViperString* s) {
    if (!s) return 0;
    /* Check bit 63 of the length field (which is at offset 8) */
    return s->data.heap.length < 0;
}
...
static inline int64_t vp_str_len_inline(ViperString* s) {
    ...
    if (vp_str_is_sso_inline(s)) {
        return (int64_t)(s->data.sso.sso_len_flags & 0x7F);
    }
...
static inline ViperString* vp_str_create_sso_small(const char* str, int64_t len) {
    ...
    /* Set SSO flag in length field (bit 63) */
    s->data.heap.length = VIPER_SSO_FLAG; 
    /* Set length in the specific flags byte */
    s->data.sso.sso_len_flags = (int8_t)(len | 0x80);
...
static inline ViperString* vp_str_create_inline(const char* str) {
    if (!str) return NULL;
...
/* Free a ViperString */
static inline void vp_str_free_inline(ViperString* s) {
    if (s) {
        vp_arc_release(s);
    }
}
...
static inline ViperString* vp_str_concat_inline(ViperString* a, ViperString* b) {
    if (!a && !b) return NULL;
    ...
    if (total_len <= VIPER_SSO_CAPACITY) {
        ...
        result->data.heap.length = VIPER_SSO_FLAG;
        result->data.sso.sso_len_flags = (int8_t)(total_len | 0x80);
...
struct ViperObject {
    void* vtable;         /* 0:  Virtual method table */
    void* data;           /* 8:  Object data */
    uint64_t _reserved;   /* 16: Padding for alignment */
};                        /* Total: 24 bytes */
...
typedef struct ViperTuple {
    ...
    int64_t* elements;    /* 8:  Array of ViperValue (tagged i64 values) */
    uint64_t _reserved;   /* 16: Padding for alignment */
} ViperTuple;             /* Total: 24 bytes header + elements */
